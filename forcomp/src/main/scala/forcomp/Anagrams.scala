...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(k => k.toLower).toList.sorted.map(_ match {case (k,v) => (k, v.size)})
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    val ret = for ((c, f) <- x if (f - ym(c) > 0)) yield  c -> (f - ym(c)) 
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsIter(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case occs => for {
        ...
        sentence <- sentenceAnagramsIter(subtract(occs, wordOccurrences(word)))
